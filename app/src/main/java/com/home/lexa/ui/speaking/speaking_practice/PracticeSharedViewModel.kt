...
class PracticeSharedViewModel : ViewModel() {
    ...
    private var activeSpeakingDayId: Long = -1L

    fun bindSpeakingDay(speakingDayId: Long, resetOnChange: Boolean = true) {
        if (speakingDayId == -1L) return
        if (activeSpeakingDayId != -1L && activeSpeakingDayId != speakingDayId && resetOnChange) {
            _sessionCache.clear()
        }
        activeSpeakingDayId = speakingDayId
    }
